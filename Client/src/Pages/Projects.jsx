...
const Projects = () => {
  ...
  return (
    ...
    >
      <motion.p
        variants={item}
        className="text-5xl mt-20 md:text-6xl font-poppins text-gray-900 dark:text-white  font-bold"
      >
        Projects
      </motion.p>
      <div className="flex flex-wrap mt-10 gap-8 px-4 md:px-10 w-full max-w-6xl">
        {projects.map((project, idx) => (
          <motion.article
            key={idx}
            variants={item}
            className="w-full max-w-sm mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transform transition duration-300"
          >
            <a
      ...
      <Link to={"/allprojects"}>
        <motion.button
          ...
          variants={item}
          className="mt-10 cursor-pointer bg-blue-600 text-white px-6 py-3 rounded-full hover:bg-blue-700 transition"
        >
          Show More Projects
        </motion.button>
      </Link>
